...
int alphabeta(const GameState &state, int depth, int a, int b, bool pa){
	...
	if(depth == 0 || nextStates.size()==0){
		...
		for(int i = 0; i< nextStates.size();++i){
			v = std::max(v,alphabeta(nextStates[i],depth-1,a,b,!pa));
			a = std::max(a,v);
			if(b<=a)
				break;
		}
		...
		for(int i = 0; i< nextStates.size();++i){
			v = std::min(v,alphabeta(nextStates[i],depth-1,a,b,!pa));
			b = std::min(b,v);
			if(b<=a)
				break;
		}
...
GameState Player::play(const GameState &pState,const Deadline &pDue)
{
    ...
    for(int i = 0; i< lNextStates.size();++i){
		int newVal= alphabeta(lNextStates[i],0,std::numeric_limits<int>::min(),std::numeric_limits<int>::max(),true);
